...
class TrajectoryComposer:
    def __init__(self, actors: int = 10, windows: int = 20, attack_ratio: float = 0.1, seed: int = 42):
    ...
    def _generate_benign_window(self, window_start: datetime, window_end: datetime):
        ...
        for _ in range(workflow_count):
            ...
            workflow_start = self.temporal.uniform_random_time(window_start, window_end)
            trigger_ref = self.provenance.benign_trigger_ref(scheduler_actor.email if scheduler_actor else actor.email)
            for step in workflow_template:
                event_time = workflow_start + timedelta(seconds=step.offset_sec)
                if event_time >= window_end: continue
                event = self._create_raw_event(actor.email, step.service_name, step.method_name, step.resource_pattern, event_time, step.log_name_suffix, trigger_ref, scheduler_actor.email if scheduler_actor else None)
    ...
    def _generate_attack_window(self, window_start: datetime, window_end: datetime):
        ...
        for _ in range(workflow_count):
            ...
            if not actor: actor = self.rng.choice(self.actors.get_all())
            workflow_start = self.temporal.uniform_random_time(window_start, window_end)
            for step in workflow_template:
                event_time = workflow_start + timedelta(seconds=step.offset_sec)
                if event_time >= window_end: continue
                event = self._create_raw_event(actor.email, step.service_name, step.method_name, step.resource_pattern, event_time, step.log_name_suffix, None)
                self.events.append(event)
    ...
    def _generate_background_noise(self, window_start: datetime, window_end: datetime):
        ...
        for _ in range(noise_count):
            ...
            if action == "gcs_read":
                event = self._create_raw_event(actor.email, "storage.googleapis.com", "storage.objects.get", "projects/_/buckets/data-bucket/objects/file.csv", event_time, "data_access", trigger_ref, scheduler_actor.email if scheduler_actor else None)
            elif action == "bq_query":
                event = self._create_raw_event(actor.email, "bigquery.googleapis.com", "jobservice.insert", "projects/synth-project/jobs/query-noise", event_time, "activity", trigger_ref, scheduler_actor.email if scheduler_actor else None)
            else:
                event = self._create_raw_event(actor.email, "storage.googleapis.com", "storage.objects.list", "projects/_/buckets/data-bucket", event_time, "data_access", None, None)
            self.events.append(event)

    def _create_raw_event(self, actor_email: str, service_name: str, method_name: str, resource_name: str, timestamp: datetime, log_name_suffix: str, trigger_ref: str | None = None, delegated_from_email: str | None = None) -> dict:
        ...
        if delegated_from_email:
            auth_info["serviceAccountDelegationInfo"] = [{"firstPartyPrincipal": {"principalEmail": delegated_from_email}}]
        event = {
            "protoPayload": {"@type": "type.googleapis.com/google.cloud.audit.v1.AuditLog", "serviceName": service_name, "methodName": method_name, "authenticationInfo": auth_info, "resourceName": resource_name, "status": {}},
            "resource": {"type": "service_account" if "serviceAccounts" in resource_name else "gcs_bucket" if "buckets" in resource_name else "bigquery" if "bigquery" in resource_name else "compute" if "instances" in resource_name else "secretmanager.googleapis.com/Secret" if "secrets" in resource_name else "other", "labels": {"project_id": "synth-project"}},
            "timestamp": ts_str,
            "insertId": insert_id,
